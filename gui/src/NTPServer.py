...
class NTPServer(object):
    def __init__(self):
        ...
        self.pid = None
        self.start_time = 0
        self.ntp = os.path.isfile(const.NTP+'/bin/ntpd')
        self.get_pid()
    ...
    def get_pid(self):
        if not self.ntp:
            logging.error('The ntp server is not installed')
            return 'Error, ntp server is not installed', 1
        try:
            self.pid = sub.check_output(['pidof', 'ntpd'])
        ...
        if self.pid:
            self.is_start = True
            return 'Ok, ntp pit = ' + self.pid, 0
        else:
            self.is_start = False
            return 'Ok, ntp pit = Null' + self.pid, 0
    ...
    def run(self, timeout=0.1):
        ...
        if not self.is_start:
            try:
                ntpd = [const.NTP+'/bin/ntpd', ]
                subPopen = sub.Popen(ntpd, cwd=const.NTP+'/bin/')
                subPopen.wait()
            except OSError, err:
            ...
            while not self.is_start:
                ...
                if now_time - self.start_time > timeout:
                    logging.error('Error to start ntp server (timeout)')
                    return 'Error timeout', 1
            logging.info('Ok, the ntp server is now running')
            with open(const.PID_NTP_FILE, 'w') as f:
                f.write(self.pid)
    ...
    def stop(self):
        ...
        if self.is_start:
            try:
                ...
                logging.error('Error to stop ntp server')
                return 'Error to stop server', 1
            logging.info('Ok, ntp server stoped')
            self.pid = None
            self.is_start = False
            try:
                os.remove(const.PID_NTP_FILE)
            except OSError, err:
                logging.error(err)
            return 'Ok, ntp server stoped', 0
        else:
            logging.info('The ntp server is not running')
            return 'Ok', 0
